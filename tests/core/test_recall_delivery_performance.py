...
_BASELINE = _ROOT / "benchmarks" / "memory" / "recall-delivery-performance-v1.json"
_RUNNER = runpy.run_path(str(_ROOT / "scripts" / "run_recall_delivery_performance.py"))


def test_recall_delivery_performance_baseline_is_complete_and_bounded() -> None:
    report = json.loads(_BASELINE.read_text(encoding="utf-8"))

    assert report["schema_version"] == "cayu.recall_delivery_performance.v1"
...
def test_recall_delivery_performance_detects_every_regression_lane() -> None:
    report = json.loads(_BASELINE.read_text(encoding="utf-8"))
