...
class PricingCatalog(BaseModel):
    ...
    def match_price(self, *, provider_name: str | None, model: str | None) -> ModelPricing | None:
        index = _best_match_index(
            self.prices,
            provider_name=provider_name,
            model=model,
            key=lambda price: (price.provider_name, price.model, price.match),
        )
        return None if index is None else self.prices[index]
...
class PriceTier(BaseModel):
    ...
    @field_validator("input_per_million", "output_per_million", "cache_read_input_per_million")
    @classmethod
    def validate_decimal(cls, value: Decimal | None, info) -> Decimal | None:
        if value is None:
            return None
...
class TieredPricing(BaseModel):
    ...
    Only the context-band ``standard`` tiers and ``cache_write_5m_per_million`` feed the
    cost engine today (via ``ModelInfo.to_model_pricing``/``pricing_at``). ``batch`` and
    ``cache_write_1h_per_million`` are carried for completeness and future use — they are
    NOT auto-applied by ``estimate_session_cost``.
    """
...
class ModelInfo(BaseModel):
    ...
    Projects into the flat cost engine via ``to_model_pricing()`` (base tier) or
    ``pricing_at(input_tokens)`` (the tier for a given step). Capabilities are
    objective facts only — no quality/benchmark scores.
    """
    ...
    @field_validator("aliases", "modalities_in", "modalities_out")
    @classmethod
    def validate_string_members(cls, value: tuple[str, ...], info) -> tuple[str, ...]:
        # These feed routing/display; every member must be a clean non-blank string.
        return tuple(require_clean_nonblank(member, info.field_name) for member in value)

    def _model_pricing(self, tier: PriceTier) -> ModelPricing:
        return ModelPricing(
            ...
            cache_write_input_per_million=self.pricing.cache_write_5m_per_million,
            currency=self.pricing.currency,
            match=self.match,
        )

    def to_model_pricing(self) -> ModelPricing:
        """Flat ``ModelPricing`` from the base tier (back-compat projection)."""
        return self._model_pricing(self.pricing.base())
...
class ModelCatalog(BaseModel):
    ...
    Projects into the flat ``PricingCatalog`` (cost engine) via ``pricing_catalog()``
    (each model's base tier) and supports capability routing. Use ``resolve()`` to find
    the record that prices a runtime model id (prefix/case-insensitive, like the cost
    engine); ``match()`` is a strict exact lookup.
    """
    ...
    def validate_unique_models(self) -> ModelCatalog:
        ...
        for info in self.models:
            key = _match_key(info.provider_name, info.model, info.match)
            if key in seen:
                raise ValueError("Model catalog contains duplicate provider/model/match entries.")
            seen.add(key)
        return self

    def pricing_catalog(self) -> PricingCatalog:
        return PricingCatalog(prices=tuple(model.to_model_pricing() for model in self.models))

    def resolve(self, *, provider_name: str | None, model: str | None) -> ModelInfo | None:
        """The most-specific record whose ``match`` rule matches ``(provider_name, model)``.

        Prefix/case-insensitive, sharing the cost engine's matcher — so a dated runtime id
        like ``claude-opus-4-8-20260101`` resolves to the ``claude-opus-4-8`` record.
        """
        index = _best_match_index(
            self.models,
            provider_name=provider_name,
            model=model,
            key=lambda info: (info.provider_name, info.model, info.match),
        )
        return None if index is None else self.models[index]
...
def _resolve_price(
    *,
    ...
    A tier-aware catalog match (the model's tier for this step's input-token count) is
    preferred over the flat ``pricing`` catalog. When both a catalog and a flat entry are
    present, a currency-matching source is preferred so a catalog record priced in another
    currency does not shadow a usable flat price; if neither matches the requested currency,
    the catalog price is returned so the caller reports the currency mismatch. ``tier_max_input_tokens``
    is the ceiling of the catalog band used (``None`` for the open-ended tier or a flat price).
    """
    candidates: list[_ResolvedPrice] = []
    if catalog is not None:
        info = _match_with_requested_fallback(
            metrics, lambda model: catalog.resolve(provider_name=metrics.provider_name, model=model)
        )
        if info is not None:
            tier = info.pricing.tier_for(metrics.input_tokens)
            candidates.append(
                _ResolvedPrice(info.pricing_at(metrics.input_tokens), tier.max_input_tokens)
            )
    if pricing is not None:
        flat = _match_with_requested_fallback(
            metrics,
            lambda model: pricing.match_price(provider_name=metrics.provider_name, model=model),
        )
        if flat is not None:
            candidates.append(_ResolvedPrice(flat, None))
    if not candidates:
        return None
...
def _match_with_requested_fallback(
    ...
    """Look up by the resolved model, then fall back to the originally requested model."""
    result = lookup(metrics.model)
    if result is None and metrics.requested_model != metrics.model:
...
def _best_match_index(
    ...
    *,
    ...
    """Index of the most-specific prefix/exact match, or ``None``.
