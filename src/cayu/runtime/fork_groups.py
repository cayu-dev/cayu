...
class ForkGroupCoordinator:
    ...
    def register_gate(self, gate_id: str, gate: ForkGroupGate) -> ForkGroupGateSelection:
        ...
        if len(gate_id.encode("utf-8")) > 256:
            raise ValueError("gate_id must be at most 256 UTF-8 bytes.")
        if not isinstance(gate, ForkGroupGate):
            raise TypeError("gate must be a ForkGroupGate.")
        gate_identity = require_durable_clean_nonblank(gate.identity, "gate.identity")
        if len(gate_identity.encode("utf-8")) > 256:
            raise ValueError("gate.identity must be at most 256 UTF-8 bytes.")
        for field_name, value in (("gate_id", gate_id), ("gate.identity", gate_identity)):
...
class ForkGroupDispositionRecord(BaseModel):
    ...
    @field_validator("branch_id", "reason")
    @classmethod
    def validate_text(cls, value: str, info) -> str:
        if info.field_name == "branch_id":
            return require_durable_clean_nonblank(value, info.field_name)
...
class _ForkGroupRecord(BaseModel):
    ...
    evaluator_execution_profile_fingerprint: str
    ...
    def validate_envelope(self) -> _ForkGroupRecord:
        ...
        if len(self.evaluator_execution_profile_fingerprint) != 64 or any(
            character not in "0123456789abcdef"
            for character in self.evaluator_execution_profile_fingerprint
        ):
...
def _event(group: _ForkGroupRecord, event_type: EventType) -> Event:
    ...
    branch_results = {branch.branch_id: branch for branch in result.branches}
    return Event(
        id=(
            "fork_group_"
            + sha256(
            ...
            "branches": [
                {
                    ...
                    "status": (
                        None
                        if (branch := branch_results.get(spec.branch_id)) is None
                        else branch.status.value
                    ),
                }
                for spec in group.request.branches
            ],
...
async def _publish_record(
    ...
    *,
    ...
    try:
        ...
        # Accept only an exact-or-newer durable revision; otherwise preserve the
        # original publication failure for the caller.
        loaded = await _load_record(coordinator, source_session_id, record.request)
        if loaded is None or loaded.revision < record.revision:
            raise
        return loaded
    loaded = await _load_record(coordinator, source_session_id, record.request)
    if loaded is None:
...
async def _run_outcome(
    ...
    *,
    ...
    return status, output if has_output else _MISSING, error
...
async def _fork_exact_source(
    ...
    *,
    ...
    try:
        ...
        return SessionStatus.FAILED, _exception_message(
            exc,
            redactor=coordinator.secret_redactor,
        )
    child = await coordinator.session_store.load(request.session_id or "")
    if child is None:
        return SessionStatus.FAILED, "Fork completed without a durable child session."
    return child.status, None
...
async def _prepare_branch_fork(
    ...
    fork_status, fork_error = await _fork_exact_source(
        coordinator,
        _branch_fork_request(request, branch),
        source=source,
    )
...
async def _run_branch(
    ...
    if child.run_epoch == 0:
        status, output, error = await _run_outcome(
            coordinator.resume(_branch_resume_request(request, branch)),
            redactor=coordinator.secret_redactor,
        )
    else:
        status = child.status
        ...
        if status is SessionStatus.FAILED:
            error = "Branch session failed before fork-group reconstruction."
        elif status is SessionStatus.INTERRUPTED:
            error = "Branch session was interrupted before fork-group reconstruction."
...
def _evidence_payload(record: _ForkGroupRecord) -> dict[str, Any]:
    ...
    evidence = {
        "schema": "cayu.fork-group-evidence.v1",
...
def _judgment_spec(branch_ids: tuple[str, ...]) -> StructuredOutputSpec:
...
def _evaluator_execution_material(
    ...
    *,
    ...
    return {
        ...
        "structured_output": _judgment_spec(branch_ids),
...
def _evaluator_run_request(
    ...
    *,
    ...
    return RunRequest(
        ...
        **_evaluator_execution_material(record, branch_ids=branch_ids),
    )
...
def _validate_judgment(
    ...
    dispositions = tuple(
        ForkGroupDispositionRecord.model_validate(item) for item in output["dispositions"]
    )
    judged_ids = [item.branch_id for item in dispositions]
    if len(judged_ids) != len(set(judged_ids)) or set(judged_ids) != set(branch_ids):
        raise ValueError("Evaluator must cover every successful branch exactly once.")
    ...
    by_id = {item.branch_id: item for item in dispositions}
    return tuple(by_id[branch_id] for branch_id in branch_ids)
...
async def _run_evaluator(
    ...
    branch_ids = tuple(branch.branch_id for branch in record.result.branches)
    ran_new = False
    output: Any = _MISSING
    error: str | None = None
    if child is None:
    ...
    if child.run_epoch == 0:
        status, output, error = await _run_outcome(
            coordinator.resume(_evaluator_resume_request(record, branch_ids=branch_ids)),
            redactor=coordinator.secret_redactor,
        )
    elif not ran_new:
        child, recovery_error = await _recover_child_for_continuation(
    ...
    try:
        dispositions = _validate_judgment(output, branch_ids)
        return (
            tuple(
                item.model_copy(
...
def _branch_failure(branches: tuple[ForkGroupBranchResult, ...]) -> ForkGroupFailure | None:
    for branch in branches:
        if branch.error is not None and "budget" in branch.error.lower():
            return _failure(
        ...
        if branch.status is ForkGroupBranchStatus.FAILED:
            code = (
                ...
                and (
                    ("source" in branch.error.lower() and "changed" in branch.error.lower())
                    or "run epoch" in branch.error.lower()
                )
                else (
                    ForkGroupFailureCode.BUDGET_EXHAUSTED
                    if branch.error is not None and "budget" in branch.error.lower()
...
async def _create_record(
    ...
    record = _ForkGroupRecord(
        ...
        request=request,
        evaluator_execution_profile_fingerprint="0" * 64,
        result=ForkGroupResult(
            group_id=request.group_id,
    ...
    if record.result.state is ForkGroupState.BRANCHES_RUNNING:
        ...
        for branch in record.request.branches:
            ...
            if outcome is not None:
                fork_failures[branch.branch_id] = outcome
        if fork_failures:
            prepared_results: list[ForkGroupBranchResult] = []
            for branch in record.request.branches:
                failed_branch = fork_failures.get(branch.branch_id)
        ...
        async def run_one(branch: ForkGroupBranchSpec) -> ForkGroupBranchResult:
            async with semaphore:
                return await _run_branch(coordinator, record.request, record.result.source, branch)

        outcomes = await asyncio.gather(
            *(run_one(branch) for branch in record.request.branches),
            return_exceptions=True,
        )
        branches: list[ForkGroupBranchResult] = []
        if any(isinstance(outcome, _ForkGroupContinuationPending) for outcome in outcomes):
            return record
        for spec, outcome in zip(record.request.branches, outcomes, strict=True):
            if isinstance(outcome, BaseException):
                ...
                branches.append(
                    ForkGroupBranchResult(
                        ...
                        session_id=spec.session_id,
                        status=ForkGroupBranchStatus.FAILED,
                        artifact_references=spec.artifact_references,
                        usage=SessionUsageSummary(session_id=spec.session_id),
                        error=_exception_message(
                            outcome,
                            redactor=coordinator.secret_redactor,
                        ),
        ...
        if failure is None:
            branch_tuple, failure = await _apply_gates(coordinator, record, branch_tuple)
