...
class WorkspaceBranchLimits(BaseModel):
    """Hard limits for one process-local workspace branch."""
...
class WorkspaceBranchPublicationRequest(BaseModel):
    ...
    @field_validator("branch_id", "baseline_revision", "change_set_digest")
    @classmethod
    def validate_text(cls, value: str, info) -> str:
        return require_durable_clean_nonblank(value, info.field_name)
...
class WorkspaceBranchRollbackResult:
    ...
    def __post_init__(self) -> None:
        ...
        if self.status is not WorkspaceBranchOutcomeStatus.ROLLED_BACK:
            raise ValueError("Workspace branch rollback result must be rolled_back.")
        if self.evidence.branch_id is None or self.evidence.baseline_revision is None:
...
class WorkspaceBranch(Workspace):
    """An isolated process-local workspace view with explicit publication."""
...
def copy_workspace_branch_request(request: object) -> WorkspaceBranchRequest:
    ...
    return WorkspaceBranchRequest(baseline=baseline, limits=envelope.limits)
