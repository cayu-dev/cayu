...
try:
    ...
    if pathlib.PurePosixPath(pattern).is_absolute() or ".." in pathlib.PurePosixPath(pattern).parts:
        fail("invalid_pattern", "Workspace list pattern must stay inside the workspace.")
    matches = []
    total_count = 0
    for path in sorted(root.rglob("*")):
        ...
        matches_pattern = (
            pathlib.PurePosixPath(rel_path).match(pattern)
            or fnmatch.fnmatchcase(rel_path, pattern)
            or (pattern.startswith("**/") and fnmatch.fnmatchcase(rel_path, pattern[3:]))
        )
        if matches_pattern:
            total_count += 1
