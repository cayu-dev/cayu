...
from cayu.delivery._git_configuration import executable_digest, require_inert_repository_config
from cayu.delivery._git_ownership import (
    RemoteGitOwnershipUnavailable,
    delivery_write_pending,
    exclusive_delivery,
)
from cayu.runners import ExecCommand, ExecResult, LocalRunner
from cayu.tasks.contracts import CompletionVerdict, WorkEvidenceReference
    ...
    async def _cleanup_delivery_root(self, request: RemoteGitDeliveryRequest, root: Path) -> bool:
        ...
        result = await self._local_runner.exec(
            ExecCommand.process(
                sys.executable,
                "-I",
                str(Path(__file__).with_name("_git_cleanup.py")),
                str(root),
            ),
        ...
        try:
            result = await runner.exec(
                ExecCommand.process(
                    sys.executable,
                    "-I",
                    str(Path(__file__).with_name("_git_process.py")),
