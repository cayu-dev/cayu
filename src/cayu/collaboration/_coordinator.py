...
from cayu.collaboration.base import IDENTITY_FAMILY, CollaborationStore
...
class ParticipantCoordinator:
    ...
    def _page(
        ...
        for count in range(len(records), -1, -1):
            ...
            value = {field: plain[:count], "next_cursor": continuation}
            try:
    ...
    def _capability(
        self, store: CollaborationStore, initialized: CollaborationInitialization, *, mutation: bool
    ) -> None:
        require_capability(
            store.capabilities(initialized.owner),
            expected_owner=initialized.owner,
            required=IDENTITY_FAMILY,
            supported=(IDENTITY_FAMILY,),
