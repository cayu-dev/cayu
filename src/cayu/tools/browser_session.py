...
class BrowserSessionTool(Tool):
    ...
    async def run(self, ctx: ToolContext, args: dict[str, Any]) -> ToolResult:
        ...
        if parent_state is None and request["operation"] != "navigate":
        ...
        try:
            async with lock:
                return await self._run_locked(ctx, parent_state, request)
        finally:
            parent_state.active_calls -= 1
    ...
    async def _run_locked(
        ...
        if operation_id is not None:
            existing = parent_state.operations.get(operation_id)
            if existing is None:
                existing = parent_state.cleanup_operations.get(operation_id)
...
def _validated_request(args: object, *, max_wait_ms: int) -> dict[str, Any]:
    ...
    common_page = {"operation", "session_id", "page_id"}
    revision_page = common_page | {"expected_revision", "operation_id"}
