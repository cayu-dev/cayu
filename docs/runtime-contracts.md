...
`CayuApp.resume(...)` resolves the current candidate before provider or tool work and compares it with the session's durable expected profile. Exact structural reuse proceeds without a profile transition. Changed, missing, or `unavailable` required identity fails closed. A changed profile emits one idempotent `session.execution_profile.rejected` event with only the expected and candidate profile fingerprints plus the changed component classes, then raises `ExecutionProfileMismatchError`; session status, run epoch, checkpoint, transcript, and interaction state remain unchanged. Repeating the same rejected boundary reuses its durable event. Existing explicit `ModelTarget` adoption is the one compatibility path in this slice: it may change only the provider-target component and advances the expected profile atomically with the existing model-switch transaction. A resumed session continues to use its already-durable system projection, so changing the live `AgentSpec.system_prompt` or workspace source alone does not claim that either was re-injected.

Custom `SessionStore` implementations must opt into `supports_execution_profile_admission`, repeat the candidate comparison inside `admit_execution_profile_resume(...)`'s status/checkpoint transaction, and implement `reject_execution_profile_resume(...)` as an atomic conditional event append. A store without those capabilities fails closed before resume work. In-memory, SQLite, and Postgres stores share conformance coverage for baseline creation, exact admission, changed-profile rejection, atomic rollback, and idempotent replay. Profile adoption beyond the existing model-target switch, historical behavior-version identity, lifecycle continuations, dispatch, verifier attempts, and complete fork lineage rules are separate contracts. Until the fork-baseline contract lands, a derived session with no profile record retains its earlier resume behavior; once a fork carries profile authority, normal admission applies and malformed authority fails closed.
