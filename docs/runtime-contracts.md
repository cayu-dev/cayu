...
Callers resolve pending approvals with `CayuApp.resolve_tool_approval(ToolApprovalRequest(...))`. Both resolution request types require `approval_id`, `tool_round_id`, and the effective `tool_call_id`. The store verifies those values against the complete paired approval/round snapshot in the same transaction that claims `interrupted → running` and advances the run epoch. The same private coordination state binds a canonical digest of the approval request's decision, reason, metadata, and resolver actor. A stale resolver therefore cannot claim a session that has paused again on another approval merely because its status is again `interrupted`, and a same-decision retry with changed audit or authority data cannot execute under an earlier grant. Approval emits `session.resumed` and emits `tool.call.approved` only for authoritatively approval-gated calls. The request digest is not included in public events: once the private intent matches, grant replay uses the event's exact approval, round, and call identities, while bounded or redacted audit fields never decide replay authority. The runtime then runs only calls with durable executable authority, appends the grouped tool-result message, clears the exact paired approval and round, and continues the model loop. Ambiguous recovery gates are deliberately approval-shaped for operator visibility, but their positive action is labelled as safe continuation in the dashboard and produces blocked results rather than authorization. Denial emits `session.resumed`, does not execute the interrupted round, appends error results for the round, clears the same exact pair, and continues the model loop. The grouped tool-result message, exact checkpoint clear, and causal `session.checkpointed` event are committed with an insert-only `approval-close:{approval_id}` runtime-publication receipt. A lost acknowledgement is reconciled from that exact receipt; an identical request returns the durable closure without rebinding an environment or dispatching a tool, while a conflicting approval identity, decision, reason, metadata, or resolver actor fails closed. Limit-triggered closure carries the original requested decision and request digest even though its operational stop decision is `limit_reached`.

When a `ResumeRequest` discovers an interrupted raw round and must first publish a recovered fail-closed approval, its new messages are retained as private deferred interaction input and referenced by the pending round instead of being appended after the unresolved assistant tool call. Resolution first publishes exactly one grouped tool-result message while atomically clearing the exact approval/round pair. Only after that durable close does it idempotently materialize the deferred input and build the next provider request. Ordinary tool-round receipts therefore describe only the grouped tool result; if a process stops between close and materialization, receipt replay or incomplete-session recovery finishes the retained tail without losing, reordering, or duplicating it. Approval retry uses stored terminal tool events for the same approval as the execution ledger: completed, failed, blocked, or approval-denied tool results are reused and never re-executed, but the exact request digest must still match before the retry can advance. An ambiguous blocked result also binds the recovery acknowledgement, so a retry cannot reinterpret it as a different decision or substitute different audit data. Durable resolution attempts, grants, denials, or terminal outcomes written before request digests existed are not used to infer request identity from their bounded and redacted event representations; automatic resolution remains interrupted before environment binding or tool execution. A pre-digest resolution intent is likewise loadable for diagnosis but is not upgraded into new authority.
