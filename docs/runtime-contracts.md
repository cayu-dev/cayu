...
`CayuApp.run_fork_group(...)` is the public in-process coordinator for one bounded candidate population. A `ForkGroupRequest` names one terminal source session, one shared `causal_budget_id`, 2-16 caller-named sibling sessions, bounded parallelism, optional registered deterministic gate selections, and one evaluator session. Each gate selection binds both its application-local ID and its stable implementation identity into the normalized request. The runtime resolves `ForkGroupCheckpointSelector` once to the source's exact status, run epoch, permanent transcript cursor and digest, semantic checkpoint digest, and effective execution-profile fingerprint. It atomically publishes that frozen authority with `fork_group.created`, then prepares every sibling fork before the first candidate model call. Each child-creation transaction revalidates the run epoch, transcript digest, semantic checkpoint digest, and effective profile against the frozen authority inside the store's atomic fork boundary; a same-epoch transcript, checkpoint, or profile mutation therefore fails closed instead of silently splitting sibling history. Fork-group coordinator records are excluded from the semantic checkpoint digest so publishing lifecycle bookkeeping does not mutate the source being frozen. Every branch is an ordinary profiled transcript/checkpoint fork, inherits the source profile unless its branch spec explicitly selects an authorized current child, and resumes through the normal session, provider, retry, limit, and causal-budget paths. Before child creation, the relationship also binds a privacy-safe digest of the exact prepared first resume and its complete execution profile. An inherited branch may differ from the source only in the request-owned invocation budget, structured-output, and finalization components; all other drift requires an explicitly authorized current-child selection. Normal resume admission adopts that frozen first profile exactly once at run epoch zero, while a different request or changed registration fails before provider dispatch. The coordinator adds no scheduler or alternate execution engine.

Fork-group lifecycle is durable on the source session as `created`, `branches-running`, `awaiting-evaluation`, and terminal `completed` or `failed`, with matching typed, schema-versioned events. Each transition increments a durable revision and uses the existing atomic session-operation record seam as a compare-and-swap boundary. Lifecycle event IDs are deterministic from group identity, request digest, revision, and event type. Before executing any nonterminal state, a coordinator atomically acquires and renews one expiring durable execution claim in that same record. Applications sharing a store therefore converge on one owner instead of concurrently resuming the same children; process loss makes the exact group eligible only after the claim expires. A stale coordinator loads the winning revision instead of overwriting it, terminal records are immutable, and a transaction that commits before losing its acknowledgement is reconciled from the exact-or-newer durable revision. Those bounded payloads name the frozen source authority, requested branch identities and current statuses, evaluator identity, terminal failure, complete dispositions, and selected branch without copying prompts or outputs. `group_id` is the idempotency key: Cayu binds it to a canonical digest of the complete normalized request, reconstructs an exact retry from the durable operation record and child sessions, and raises `ForkGroupConflict` if the same group ID carries different source, branch, gate, evaluator, budget, or limit material. The in-memory, SQLite, and PostgreSQL session stores all use this same contract. A fresh `CayuApp` around the same durable store can return a completed or failed group without provider access. During nonterminal reconstruction, any previously started nonterminal branch or evaluator first enters Cayu's ordinary incomplete-session recovery path. An exact provider operation that remains pending leaves the group in `branches-running` or `awaiting-evaluation` so a later identical request can continue it; recovered terminal failure, interruption, or invalid evidence still fails version 1 closed. Continuing a nonterminal group additionally requires the same agents and exact deterministic gate identities.

Application code registers a `ForkGroupGate` under each stable ID and receives the exact `ForkGroupGateSelection` to place in the request. After every candidate session completes successfully, Cayu verifies that the registered handler still exposes the selected implementation identity, then passes it a bounded `ForkGroupGateRequest` containing only the frozen source identity and copied branch result. Gate handlers return a bounded `ForkGroupGateDecision`; Cayu persists the resulting pass/fail records and never lets a model override them. A missing, changed, throwing, malformed, or failing gate fails the group before evaluation. Gates are application-owned deterministic checks, not model judges or a security boundary.

Only when every branch completes with validated structured output or declared artifact references and every deterministic gate passes does Cayu create the evaluator as a fresh session on the same causal budget. It derives an internal agent from the registered evaluator but removes all application tools, workflow tool names, runtime hooks, and loop policies. The coordinator resolves and persists that exact tool-free evaluator execution-profile fingerprint before publishing the group; restart or continuation fails closed if mutable registrations would produce another profile. The evaluator receives a runtime-generated, size-bounded `cayu.fork-group-evidence.v1` document containing only source digests, branch/session IDs, terminal statuses, validated structured output or declared artifact references, usage, and gate results; it receives no source or branch transcript. Its runtime-owned structured schema must cover every successful branch exactly once, use only `selected`, `rejected`, `advisory`, or `archived`, and mark exactly one branch `selected`. Disposition records are frozen after validation. Runtime-owned source-snapshot authority is attached to each fork only after ordinary workload metadata redaction, so a configured secret that happens to equal a status or other schema value cannot corrupt the child relationship.

Version 1 fails closed: one failed, interrupted, invalid, oversized, budget-exhausted, or gate-failing branch prevents evaluator execution and terminalizes the group as failed while preserving all sibling sessions and evidence. Evaluator failure or invalid judgment likewise preserves every candidate and fails the group. Extension failures are projected through bounded, secret-redacted diagnostics before entering branch results or the durable group record. Evaluating viable siblings after a failure, replacement candidates, task-backed/distributed execution, workspace merge/promotion/deletion, and remote copy-on-write branches remain separate capabilities.
