...
Approval, user-input, tool-round, and incomplete-session continuation reload that record before provider reconciliation, environment binding, tool execution, or model dispatch. Recovery requires the pending records to reference the active fingerprint and requires the current process to reconstruct the same provider target and direct-tool profile; it then atomically rebinds the unchanged profile to the continuation epoch. A missing, malformed, incompatible, stale-epoch, or differently referenced snapshot fails closed before governed work and profile drift records the ordinary durable rejection evidence.
