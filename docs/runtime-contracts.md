...
`checkpoint_schema_version`. The current version is `2` and the minimum
...
Custom `SessionStore` implementations must opt into `supports_execution_profile_admission`, repeat the complete decision/profile comparison inside `admit_execution_profile_resume(...)`'s status/checkpoint transaction, append accepted decision evidence before other admission events, and implement `reject_execution_profile_resume(...)` as an atomic conditional event append. A store without those capabilities fails closed before resume work. In-memory, SQLite, and Postgres stores share conformance coverage for baseline creation, exact admission, changed-profile rejection, accepted adoption, immutable baseline/advanced expectation, atomic event ordering, rollback, and replay. Historical behavior-version identity, lifecycle continuations already past the safe resume boundary, verifier attempts, and complete fork lineage rules are separate contracts. Until the fork-baseline contract lands, a derived session with no profile record retains its earlier resume behavior; once a fork carries profile authority, normal admission applies and malformed authority fails closed.
