...
- Legacy event and transcript rows remain readable with null attribution.
...
Custom `SessionStore` implementations must also implement the interaction-admission transaction. `create(...)` and `transition_status_and_checkpoint(...)` accept optional interaction admission data and must persist the run-epoch claim, `interaction.started` side-effect handoff, and either attributed transcript messages or a private deferred source batch as one commit. The transition can instead name an existing open interaction when crash recovery must precede the admitted source batch. `admit_interaction(...)` provides the same event/source atomicity for already-owned queued-input boundaries. `replace_initial_transcript_messages(...)` materializes the ordered initial transcript without an externally visible intermediate form, `materialize_deferred_interaction_input(...)` appends the source-only fallback used before terminalization and after owned recovery, and `load_deferred_interaction_input(...)` exposes a detached snapshot for complete export. These methods are required; a store that cannot provide their atomicity is not interaction-safe.
...
JSONL is available from bounded CLI inspection and complete storage exports. A session export retains the legacy bare `transcript` message array and adds `transcript_records`, whose records carry stable absolute indices and nullable `interaction_id` values. It also includes any private deferred interaction input in addition to visible transcript, events, and checkpoint. Legacy exports without either additive field remain importable, with transcript attribution explicitly unavailable rather than inferred. JSONL should not be the primary Cayu session store because dashboards, replay, task orchestration, retries, and hosted runtimes need indexed structured queries and transactional state updates.
...
Receipt verification also re-reads the immutable durable material in the same backend lock or transactional snapshot: the exact transcript slice between the receipt cursors must retain its digest, appended events are resolved in receipt order and must retain their batch digest, and every referenced event must still exist with the exact content digest recorded in its ordered reference. Missing, reordered, or changed material fails closed instead of suppressing reconstruction. The checkpoint digest is commit-time evidence only: checkpoints are mutable snapshots that later legitimate runtime transitions replace, so a historical receipt cannot require the current checkpoint to retain that digest.
...
conservative around runtime publications. An active model-completion stage, a
checkpoint-backed pending tool round, or any immutable publication receipt pins
the affected recovery material and makes maintenance return zero for that
session. Version-1 receipts use positional transcript cursors and content-bound
event references, so a session with a committed receipt remains pinned rather
than weakening exact replay. Applications that require physical reclamation
must explicitly delete a quiescent session; future reference-aware archival can
relax this restriction without changing receipt semantics.
