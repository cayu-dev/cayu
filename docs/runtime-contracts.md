...
`CayuApp.run_fork_group(...)` is the public in-process coordinator for one bounded candidate population. A `ForkGroupRequest` names one terminal source session, one shared `causal_budget_id`, 2-16 logical candidate slots with caller-named initial sessions, bounded parallelism, optional registered deterministic gate selections, an explicit `ForkGroupFailurePolicy`, and one evaluator session. `fail-group` is the compatible default. `evaluate-viable` additionally binds a minimum viable count, a group-wide replacement-attempt limit, replacement parallelism, deterministic gates, and one registered replacement planner selection. Gate and planner selections bind both an application-local ID and a stable implementation identity into the normalized request. The runtime resolves `ForkGroupCheckpointSelector` once to the source's exact status, run epoch, permanent transcript cursor and digest, semantic checkpoint digest, and effective execution-profile fingerprint. It atomically publishes that frozen authority with `fork_group.created`, then prepares every initial sibling fork before the first candidate model call. Each child-creation transaction revalidates the run epoch, transcript digest, semantic checkpoint digest, and effective profile against the frozen authority inside the store's atomic fork boundary; a same-epoch transcript, checkpoint, or profile mutation therefore fails closed instead of silently splitting sibling history. Fork-group coordinator records are excluded from the semantic checkpoint digest so publishing lifecycle bookkeeping does not mutate the source being frozen. Every attempt is an ordinary profiled transcript/checkpoint fork and resumes through the normal session, provider, retry, run-limit, and causal-budget paths. Before child creation, the relationship also binds a privacy-safe digest of the exact prepared first resume and its complete execution profile. The coordinator adds no population scheduler or alternate execution engine.

Fork-group lifecycle is durable on the source session as `created`, `branches-running`, `awaiting-evaluation`, and terminal `completed` or `failed`, with matching typed, schema-versioned events. Each transition increments a durable revision and uses the existing atomic session-operation record seam as a compare-and-swap boundary. Lifecycle event IDs are deterministic from group identity, request digest, revision, and event type. Before executing any nonterminal state, a coordinator atomically acquires and renews one expiring durable execution claim in that same record. Applications sharing a store therefore converge on one owner instead of concurrently creating replacements or resuming the same children; process loss makes the exact group eligible only after the claim expires. A stale coordinator loads the winning revision instead of overwriting it, terminal records are immutable, and a transaction that commits before losing its acknowledgement is reconciled from the exact-or-newer durable revision. Schema-v3 lifecycle payloads identify every attempt, its digest-bound prepared request, logical slot and index, replacement edges, session, status, eligibility, evaluator identity, terminal failure, complete dispositions, and selected attempt without copying prompts or outputs. `group_id` is the idempotency key: Cayu binds it to a canonical digest of the complete normalized request, reconstructs an exact retry from the durable operation record and child sessions, and raises `ForkGroupConflict` if the same group ID carries different source, branch, gate, planner, evaluator, budget, or limit material. In-memory, SQLite, and PostgreSQL stores use this same contract. A fresh `CayuApp` around the same store returns a terminal group without provider or planner access. During nonterminal reconstruction, a previously started nonterminal attempt or evaluator first enters ordinary incomplete-session recovery. A still-active provider operation leaves the group nonterminal for a later identical request; continuing additionally requires the same agent, gate, and planner identities.
...
An `in_flight` model-completion stage is dispatch-ambiguous. It proves that the runtime crossed its durable preparation boundary, but it does not prove whether provider-controlled billable work began or whether a terminal response existed only in lost process memory. Generic recovery must not treat it as permission to dispatch the provider again. Promotion raises `SessionModelCompletionStageIncomplete`; runtime integration must fail closed into conservative accounting/manual reconciliation unless it has separate trusted evidence that dispatch never occurred. Provider errors and cancellation before a terminal `model.completed` record therefore remain in this ambiguous state once dispatch may have begun.

`abandon_model_completion_stage(...)` exists only for the narrower case where runtime-owned control flow proves dispatch never began after preparation. It atomically matches the active stage id, preparation digest, and run epoch; refuses any terminal stage, logical-step winner, or publication receipt; writes an immutable content-bound abandonment tombstone; and removes only that exact active preparation. Exact acknowledgement-loss replay returns the same evidence, while a later re-preparation cannot be deleted by a stale abandonment request. The store supplies these fences but cannot infer the external provider boundary itself, so callers must never use abandonment to resolve an ambiguous dispatched stage.
...
This does not disable ordinary live retries: code that directly observes a terminal provider error in the same process may prepare a greater dispatch ordinal under a distinct stage id and the same logical step id, while the failed attempt's stage remains `in_flight`. That live observation, not the stranded stage itself, authorizes the retry; after process loss the same state remains fail closed. If a late response for the earlier attempt arrives after the retry publishes, Cayu retains its terminal evidence but winner verification rejects promotion, so it cannot add a contradictory assistant turn or duplicate completion event.
