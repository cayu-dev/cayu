...
`checkpoint_schema_version`. The current version is `4` and the minimum
...
Interaction pause and terminal publication uses `SessionStore.publish_interaction_transition()`. The store commits the lifecycle event, its side-effect handoff, the corresponding session-status decision, and an immutable transition receipt in one transaction. A queue-guarded completion always closes the finished interaction but leaves the session running when queued input remains. Repeating the exact event ID reconstructs the original committed session/result snapshot without consulting later session state or re-evaluating the queue decision; reusing the ID with different transition data fails closed. The receipt retains and content-binds the event and historical session snapshot, so exact replay does not depend on ordinary event retention and structurally valid receipt drift cannot become authoritative. `load_interaction_transition_receipt()` performs the corresponding read-only exact lookup for cancellation reconciliation and never creates a missing transition. Receipt replay remains run-epoch fenced: a worker that lost ownership cannot use idempotency to continue after its epoch was superseded. Custom stores must provide the same atomic, idempotent, and read-only receipt contract.
...
Custom `SessionStore` implementations must opt into `supports_execution_profile_admission`, repeat the complete decision/profile comparison inside `admit_execution_profile_resume(...)`'s status/checkpoint transaction, append accepted decision evidence before other admission events, and implement `reject_execution_profile_resume(...)` as an atomic conditional event append. Active invocation authority is a separate protocol capability: a store must also opt into `supports_active_invocation_execution_profiles`, implement the typed `admit_session_invocation(...)` checkpoint/status compare-and-swap, and implement `reject_active_invocation_execution_profile(...)` as an atomic comparison of the expected checkpoint snapshot plus conditional event append. A legacy custom store that exposes only the earlier admission capability fails closed through the new base methods; Cayu does not call its older rejection override with an expanded signature. In-memory, SQLite, and Postgres stores share conformance coverage for baseline creation, exact admission, changed-profile rejection, active continuation and rejection, accepted adoption, immutable baseline/advanced expectation, atomic event ordering, rollback, and replay. Historical behavior-version identity, contracted verifier-attempt carriage, and complete fork lineage rules are separate contracts. Every runtime operation that executes or derives a durable session—including resume, recovery, explicit compaction, and fork—requires complete execution-profile and tool-capability-ceiling authority. Cayu never reconstructs missing authority from the current agent registration; an incomplete record fails closed before admission or descendant creation.
