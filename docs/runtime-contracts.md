...
`Workspace.create_branch(WorkspaceBranchRequest(...))` is an optional capability whose compatibility default returns a typed `unsupported` result. The exact built-in `LocalWorkspace` implements the first supported slice. A successful request must carry one complete, supported revision observation for that same workspace. Creation takes exclusive cooperative ownership of the source, streams a bounded private baseline, recomputes the deterministic workspace manifest revision, and returns `conflicted` rather than opening a branch when the source no longer matches. Runner-backed, remote, sandbox, Git, invocation-facade, and unproven `LocalWorkspace` subclass implementations remain unsupported.
...
Durable local branches are an explicit opt-in. Construct `LocalWorkspace(..., branch_store=SessionWorkspaceBranchStore(session_store), branch_authority_resolver=WorkspaceBranchBindingAuthorityRegistry(...))` and supply `branch_id`, `idempotency_key`, and `WorkspaceBranchAuthority` together on `WorkspaceBranchRequest`. `WorkspaceBranchStore` is the narrow workspace-owned record-journal contract; `SessionWorkspaceBranchStore` is the runtime adapter that binds it to guarded `SessionStore` transitions without making the workspace layer depend on runtime storage types. A custom `SessionStore` must advertise `supports_owned_off_thread_session_commit_guards=True` only when its guarded publication keeps the synchronous mutation owned through cancellation and runs it away from the asyncio event-loop thread; a subclass that overrides guarded publication must redeclare that capability for its exact implementation. The adapter rejects unsupported or merely inherited capability claims before accepting branch state. The authority provider returns the live `WorkspaceBranchBindingAuthority` and owns generation claims; Cayu holds one exact generation claim from durable admission through private or source mutation and terminal settlement and treats that successful claim as authority evidence for that operation. A custom provider returns a transferable `WorkspaceBranchBindingAuthorityClaim`, whose idempotent `release()` may run from a later task or cleanup owner and must keep the claim active when release raises so Cayu can retain and retry it. The guarded operation neither re-enters the provider nor calls it from an off-thread commit guard while its claim is active; independent concurrent operations remain separate claims. `WorkspaceBranchBindingAuthorityRegistry.replace()` rejects replacement while such a claim is active, and an old handle cannot keep acting after replacement. The creation authority binds the owning session and run epoch, environment, workspace-binding generation and identity, creating authority, and resource policy. Cayu stores a digest-protected lifecycle record under a branch-derived session-operation key before creating private material. The record retains the source and baseline revision, exact root identity, limits and expiry, authority-namespaced private sibling and capture-attempt identities, overlay identities, deletion tombstones, publication or rollback authority, and a monotonic record revision. Supplying only part of this authority is rejected; reusing a branch or operation identity with different material raises `WorkspaceBranchOperationConflict`.
