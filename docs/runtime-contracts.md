...
Terminal environment binding finalization performs one separately bounded revision observation after mutation-quiescence fencing and before the binding's final mutation or synchronization step. If a custom final observer does not settle after its deadline or caller abandonment, its exact task transfers to the environment mutation fence; binding finalization, abandonment, and later reuse remain blocked until that task positively settles. For an environment with a positively static secret-resolution scope, `environment.binding.finalize.completed.final_revision` reports the last safely observable workspace ID, observer, status, revision, HEAD, branch, scope, count, and fixed detail code. A vault-, proxy-, or factory-backed environment instead publishes fixed content-free `truncated` evidence with no revision, HEAD, or branch and omits the sibling final snapshot: terminal finalization may run after the invocation redactor has been discarded or in a fresh process, so it cannot reconstruct dynamic secrets safely in either representation. If the subsequent binding finalization or finalize-completion publication fails, the same bounded safe projection is retained on the failure evidence and terminal session event instead of being discarded. It contains no tool-call or mutation-window identity, so it remains useful when per-tool evidence is incomplete without claiming that later or overlapping changes belonged to a particular tool.
