...
stream protocol, and bounded control state; private recovery metadata is never
exposed. Version `6` retains version `5`'s reconnect states, version `4`'s
`synchronous` state and provider-operation object, version
