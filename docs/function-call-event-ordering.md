# Function-call event ordering diagnostics
...
there was no pending function at the received output index.
...
The parser distinguishes consumed registration from an orphan, rejects changes to
...
| Sequence or mutation | Expected behavior |
| --- | --- |
...
## Structural evidence and fault attribution

Function protocol errors now reuse the bounded trace format from
[the hosted-search diagnostics](hosted-search-event-ordering.md). The scalar
...
Attributing a failure requires bounded structural order/index/identity-relation
evidence at upstream output, any proxy ingress/egress, and native adapter ingress for the same
...
unknown-attempt cap. Synthetic controls do not establish the origin of an
unobserved malformed stream.
