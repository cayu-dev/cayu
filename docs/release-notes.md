...
built and maintained by coding agents.

- Independently dispatchable session forks, child-completion discovery, shared
  artifacts, and portable agent bundles.
- Bounded memory deltas, checkpoint-aware recall, reviewed knowledge maintenance,
  and semantic watches.
- Large tool catalogues, targeted grants, live MCP refresh, and hard Linux
  deadlines for isolated host tools.
- Workflow and multi-stage evals, structured judges, and repeated-trial results.
- More reliable recovery, workspace publication, and Docker browser reconnect.
- Refreshed model capabilities, pricing, and patched dependencies.

Compatibility is still evolving. This release uses storage revision **82**,
server contract **44**, and manifest/generator schema **17**. Update the whole
application together. Older populated stores may require recreation; use
`cayu storage status` to check. `ForkGroup` is removed: compose ordinary child
sessions instead.
